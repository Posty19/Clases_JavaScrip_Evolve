...
document.getElementById("btnCrear").addEventListener("click", (e) => {
  ...
  const pLength = document.querySelectorAll(
    "#contenedorNuevos p.elemento"
  ).length;
  //creacion del nodo texto
  const textCreation = document.createTextNode(
    `Elemento creado con numero: ${pLength + 1}`
  );
...
document.getElementById("btnInsertarAntes").addEventListener("click", (e) => {
  ...
  //creacion del nodo texto
  const textCreation = document.createTextNode(
    `Elemento creado antes del elemento de referencia`
  );
  //creacion del parrafo
  const pElement = document.createElement("p");
  //asignacion de la clase elemento
  ...
  //añadir el parrafo al contenedor
  document
    .getElementById("contenedorInserciones")
    .insertBefore(pElement, reference);
});
...
document.getElementById("btnInsertarDespues").addEventListener("click", (e) => {
  ...
  //creacion del nodo texto
  const textCreation = document.createTextNode(
    `Elemento creado despues del elemento de referencia`
  );
  //creacion del parrafo
  const pElement = document.createElement("p");
  //asignacion de la clase elemento
...
document.getElementById("btnEliminar").addEventListener("click", (e) => {
  const lastElement=document.getElementById("contenedorEliminables").lastElementChild
  lastElement? lastElement.remove():console.log('no quedan elementos por eliminar');
});

//*******************************************Reemplazar elemento
let counter = 0;
document.getElementById("btnReemplazar").addEventListener("click", (e) => {
  const newElemnt = document.createElement("p");
  newElemnt.setAttribute("class", "elemento");
  const textP = document.createTextNode(
    `Elemento reemplazado numero: ${++counter}`
  );
  newElemnt.append(textP);
  const replazableElement = document.querySelector("#contenedorReemplazo p");

  replazableElement.replaceWith(newElemnt);
});